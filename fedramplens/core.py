...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'FR-BND-001', 'severity': 'high', 'weight': 2.5, 'title': 'COMPONENT_OUT_OF_BOUNDARY', 'when': {'in_boundary': False, 'production': True}, 'remediation': 'Either add to authorization boundary or remove from prod traffic.'}, {'id': 'FR-INH-001', 'severity': 'medium', 'weight': 2.0, 'title': 'NO_INHERITANCE_RECORDED', 'when': {'inherits_from': '^$'}, 'remediation': 'Document inheritance from FedRAMP-authorized service.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="fedramp",
                    metadata=item,
                ))
